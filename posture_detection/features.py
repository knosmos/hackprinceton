...
def parameterize_ml(image):
    ...
    # Add this to all coordinates
    features = [shoulder_angle, head_tilt_angle, neck_tilt_angle, nose_eye_tilt, nose_offset]
...
def parameterize_heuristics(image):
    # Path
    if isinstance(image, str):
        landmarks = get_all_landmarks_path(image)
    # Frame
    ...
    score -= 1.15 * max(shoulder_angle - 3, 0)
    score -= 1.15 * max(head_tilt_angle - 3, 0)
    ...
    score -= 1.05 * max(neck_tilt_angle - 5, 0)
    ...
    score -= 1.05 * max(nose_eye_tilt_angle - 10, 0)

    # Head pitch angle: looking up/down
    pitch_angle = math.degrees(math.atan2(nose.y - eyes_midpoint_y, nose.x - eyes_midpoint_x))
    
    return [max(score, 0), shoulder_angle, head_tilt_angle, neck_tilt_angle, nose_eye_tilt_angle]